local MenuBind = Enum.KeyCode.End
...
do
    ...
    function utility:ScreenSize()
        return game:GetService("Workspace"):FindFirstChildOfClass("Camera").ViewportSize
    end
...
function library.New(self, info, theme)
    ...
    function window.UpdateWindowColor(self)
        main_frame_accent.Color = window.theme.accent
        tab_frame_accent.Color = window.theme.accent
    end
    ...
    function window.Tab(self, info)
        ...
        function tab.Section(self, info)
            ...
            function section.UpdateSectionColor(self)
                section_accent.Color = window.theme.accent
                section_accent2.Color = window.theme.accent
            end
            ...
            function section._Keybind(self, info, offsets, parent, do_update, pointer, name2)
                ...
                function keybind.Get(self)
                    return {self.value, library.flags[self.flag]}
                end
                ...
                function keybind.GetPointer(self)
                    return keybind.pointer
                end
            ...
            function section.Slider(self, info)
                ...
                function slider.UpdateSliderColor(self)
                    slider_bar.Color = window.theme.accent
                end
            ...
            function section.List(self, info)
                ...
                function list.UpdateScrollbarColor(self)
                    list_frame_scrollbar.Color = window.theme.accent
                end
    ...
    function window.Keybinds(self)
        ...
        function kblist.UpdateKbListColor()
            kblist_accent.Color = window.theme.accent
        end

        function kblist.Update(self)
            kblist:UpdateKbListColor()
    ...
    function window.Watermark(self)
        ...
        local watermark = {text = "sises", visible = false, instances = {}}
        ...
        function watermark.UpdateWatermarkColor()
            watermark_accent.Color = window.theme.accent
        end

        function watermark.Update(self)
            watermark:UpdateWatermarkColor()
    ...
    function window.Cursor(self)
        ...
        function cursor.UpdateCursorColor(self)
            triangle.Color = window.theme.accent
        end

        function cursor.Update(self)
            cursor:UpdateCursorColor()
...
--[[
        ...
        function self.load_config(self, data)
...
local WindowName = "sises"

    local Window = library:New({name = tostring(WindowName)})

local AimbotTab = Window:Tab({name = "Legit"})
...
local AimEnabled = AimAssistSection:Toggle({name = "Enabled", def = MainTable.Combat.AimAssist.Main.Enabled, flag = "AimbotFlag", callback = function(v) 
    MainTable.Combat.AimAssist.Main.Enabled = v
end}):Keybind({name = "BouiBot", def = MainTable.Combat.AimAssist.Main.Bind, mode = "toggle", flag = "AimbotKeybind"})

AimAssistSection:Slider({name = "Aim Range", min = 0, max = 2500, dec = 1, suf = "m", def = MainTable.Combat.AimAssist.Main.Range, callback = function(v)
    MainTable.Combat.AimAssist.Main.Range = v
end})

local Hitbox = AimAssistSection:Dropdown({name = "Hitbox", options = GetAllBodyParts(plrs.LocalPlayer), def = MainTable.Combat.AimAssist.Main.Hitbox, multi = false, scrollable = true, vis = 6, min = 1, callback = function(y)
    MainTable.Combat.AimAssist.Main.Hitbox = y
end})

AimAssistSection:Dropdown({name = "Checks", options = {"Team", "ForceField", "Visible", "Wall", "Arm"}, def = MainTable.Combat.AimAssist.Main.Checks, multi = true, scrollable = true, vis = 5, min = 0, callback = function(y)
    MainTable.Combat.AimAssist.Main.Checks = y
end})

AimAssistSection:Toggle({name = "Deadzone", def = MainTable.Combat.AimAssist.Main.UseDeadzone, flag = "Dead", callback = function(v) 
    MainTable.Combat.AimAssist.Main.UseDeadzone = v
end})

AimAssistSection:Slider({name = "Offset X", min = -50, max = 50, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Main.OffsetX, callback = function(v)
    MainTable.Combat.AimAssist.Main.OffsetX = v
end})

AimAssistSection:Slider({name = "Offset Y", min = -50, max = 50, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Main.OffsetY, callback = function(v)
    MainTable.Combat.AimAssist.Main.OffsetY = v
end})

local HorizontalSmoothness = AimAssistSection:Slider({name = "Horizontal Smoothing", min = 1, max = 100, dec = 1, suf = "%", def = MainTable.Combat.AimAssist.Main.HorizontalSmoothness, callback = function(v)
    MainTable.Combat.AimAssist.Main.HorizontalSmoothness = v
end})

local VerticalSmoothness = AimAssistSection:Slider({name = "Vertical Smoothing", min = 1, max = 100, dec = 1, suf = "%", def = MainTable.Combat.AimAssist.Main.VerticalSmoothness, callback = function(v)
    MainTable.Combat.AimAssist.Main.VerticalSmoothness = v
end})

local VisualsASection = AimbotTab:Section({name = "Visuals", side = "right"})

local ShowFov = VisualsASection:Toggle({name = "Draw FOV", def = MainTable.Combat.AimAssist.Visuals.FOV.Enabled, callback = function(v)  
    MainTable.Combat.AimAssist.Visuals.FOV.Enabled = v
end})

local FovColor = ShowFov:Colorpicker({name = "Bruh", def = MainTable.Combat.AimAssist.Visuals.FOV.Color, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.FOV.Color = v
end})

local Radius = VisualsASection:Slider({name = "Radius", min = 1, max = 180, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.FOV.Radius, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.FOV.Radius = v*3
end})

local FOVOffsetX = VisualsASection:Slider({name = "Offset X", min = -100, max = 100, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.FOV.OffsetX, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.FOV.OffsetX = v
end})

local FOVOffsetY = VisualsASection:Slider({name = "Offset Y", min = -100, max = 100, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.FOV.OffsetY, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.FOV.OffsetY = v
end})

VisualsASection:Toggle({name = "Draw Deadzone FOV", def = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Enabled, callback = function(v)  
    MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Enabled = v
end}):Colorpicker({name = "Bruh", def = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Color, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Color = v
end})

VisualsASection:Slider({name = "Radius", min = 1, max = 180, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Radius, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Radius = v*3
end})

VisualsASection:Slider({name = "Offset X", min = -100, max = 100, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetX, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetX = v
end})

VisualsASection:Slider({name = "Offset Y", min = -100, max = 100, dec = 1, suf = "px", def = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetY, callback = function(v)
    MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetY = v
end})

local TracingSection = AimbotTab:Section({name = "Tracing Configuration", side = "left"})

local Prediction = TracingSection:Toggle({name = "Player Prediction", def = MainTable.Combat.AimAssist.Tracing.Prediction.Enabled, callback = function(v)  
    MainTable.Combat.AimAssist.Tracing.Prediction.Enabled = v
end})

local PredictionAmount = TracingSection:Slider({name = "Prediction Amount", min = 1, max = 100, dec = 1, suf = "%", def = MainTable.Combat.AimAssist.Tracing.Prediction.Amount, callback = function(v)
    MainTable.Combat.AimAssist.Tracing.Prediction.Amount = v/500
end})

local BackTracing = TracingSection:Toggle({name = "Back Tracing", def = MainTable.Combat.AimAssist.Tracing.Prediction.BackTracing, callback = function(v)  
    MainTable.Combat.AimAssist.Tracing.Prediction.BackTracing = v
end})

local TriggerBotSection = AimbotTab:Section({name = "Trigger bot", side = "right"})

local TriggerBotEnabled = TriggerBotSection:Toggle({name = "Enabled", def = MainTable.Combat.TriggerBot.Main.Enabled, callback = function(v)  
    MainTable.Combat.TriggerBot.Main.Enabled = v
end}):Keybind({name = "BouiBot", def = MainTable.Combat.TriggerBot.Main.Bind, mode = "toggle", flag = "TriggerBotBind"})

TriggerBotSection:Dropdown({name = "Checks", options = {"Team", "ForceField", "Wall"}, def = MainTable.Combat.TriggerBot.Checks.Existing, multi = true, scrollable = false, vis = 2, min = 0, callback = function(y)
    MainTable.Combat.TriggerBot.Checks.Existing = y
end})

local AutoADS = TriggerBotSection:Toggle({name = "Auto ADS", def = MainTable.Combat.TriggerBot.ADS.Enabled, callback = function(v)  
    MainTable.Combat.TriggerBot.ADS.Enabled = v
end})

local Delay = TriggerBotSection:Slider({name = "ADS Delay", min = 0, max = 5000, dec = 1, suf = "ms", def = MainTable.Combat.TriggerBot.ADS.Delay, callback = function(v)
    MainTable.Combat.TriggerBot.ADS.Delay = v/1000
end})

local AutoADSType = TriggerBotSection:Dropdown({name = "Type", options = {"Hold", "Toggle"}, def = MainTable.Combat.TriggerBot.ADS.Type, mutli = false, scrollable = false, vis = 2, min = 2, callback = function(y)
    MainTable.Combat.TriggerBot.ADS.Type = y
end})

function TriggerTeamCheck(v)
    if table.find(MainTable.Combat.TriggerBot.Checks.Existing, "Team") then
        if v.TeamColor ~= plrs.LocalPlayer.TeamColor then
            return true
        else
            return false
        end
...
function TriggerWallCheck(part, ignore) 
    if table.find(MainTable.Combat.TriggerBot.Checks.Existing, "Wall") then
        Origin = plrs.LocalPlayer.Character:FindFirstChild("Head").Position
    ...
    function TriggerForceFieldCheck(Plr)
        if table.find(MainTable.Combat.TriggerBot.Checks.Existing, "ForceField") then
...
local function gettarget(instance)
   for i,v in next, Players:GetPlayers() do
    if TriggerTeamCheck(v) and MainTable.Combat.TriggerBot.Main.Enabled and v ~= plrs.LocalPlayer and plrs.LocalPlayer.Character ~= nil and v.Character ~= nil and v.Character:FindFirstChild("Head") and TriggerForceFieldCheck(v) and TriggerWallCheck(v.Character:FindFirstChild("Head").Position,{plrs.LocalPlayer.Character,v.Character}) and plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") and plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
...
game:GetService("RunService").RenderStepped:Connect(function()
    task.wait()
    if gettarget(mouse.Target) and getgenv().FrameVisible ~= false and plrs.LocalPlayer.Character ~= nil then
        if MainTable.Combat.TriggerBot.ADS.Enabled then
            if MainTable.Combat.TriggerBot.ADS.Type == "Hold" then
                mouse2press()
            elseif MainTable.Combat.TriggerBot.ADS.Type == "Toggle" then
                mouse2click()
            end
        end
        mouse1press()
        ...
        mouse1release()
        if MainTable.Combat.TriggerBot.ADS.Enabled then
            task.wait(MainTable.Combat.TriggerBot.ADS.Delay)
            if MainTable.Combat.TriggerBot.ADS.Type == "Hold" then
                mouse2release()
            elseif MainTable.Combat.TriggerBot.ADS.Type == "Toggle" then
                mouse2click()
...
local CC = game:GetService("Workspace"):FindFirstChildOfClass("Camera")
local Mouse = plrs.LocalPlayer:GetMouse()
local CurrentCamera = game:GetService("Workspace"):FindFirstChildOfClass("Camera")
local Players = game.GetService(game, "Players")
local LocalPlayer = Players.LocalPlayer

local FovOutline = Draw("Circle")
function UpdateFOVOutline()
    if MainTable.Combat.AimAssist.Main.Enabled then
FovOutline.Visible = MainTable.Combat.AimAssist.Visuals.FOV.Enabled
    else
FovOutline.Visible = false
    end
FovOutline.Radius = MainTable.Combat.AimAssist.Visuals.FOV.Radius
FovOutline.Thickness = 3
FovOutline.Color = c3rgb(255, 153, 255)
FovOutline.Position = v2new(Mouse.X+MainTable.Combat.AimAssist.Visuals.FOV.OffsetX, Mouse.Y+35+MainTable.Combat.AimAssist.Visuals.FOV.OffsetY)
end

local Fov = Draw("Circle")
function UpdateFOV()
    if MainTable.Combat.AimAssist.Main.Enabled then
Fov.Visible = MainTable.Combat.AimAssist.Visuals.FOV.Enabled
    else
        Fov.Visible = false
    end
Fov.Radius = MainTable.Combat.AimAssist.Visuals.FOV.Radius
Fov.Thickness = 1
Fov.Color = MainTable.Combat.AimAssist.Visuals.FOV.Color
Fov.Position = v2new(Mouse.X+MainTable.Combat.AimAssist.Visuals.FOV.OffsetX, Mouse.Y+35+MainTable.Combat.AimAssist.Visuals.FOV.OffsetY)
end

local DeadzoneFovOutline = Draw("Circle")
function UpdateDeadzoneFOVOutline()
    if MainTable.Combat.AimAssist.Main.Enabled and MainTable.Combat.AimAssist.Main.UseDeadzone then
        DeadzoneFovOutline.Visible = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Enabled
    else
        DeadzoneFovOutline.Visible = false
    end
    DeadzoneFovOutline.Radius = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Radius
    DeadzoneFovOutline.Thickness = 3
    DeadzoneFovOutline.Color = c3rgb(0,0,0)
    DeadzoneFovOutline.Position = v2new(Mouse.X+MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetX, Mouse.Y+35+MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetY)
end

local DeadzoneFov = Draw("Circle")
function UpdateDeadzoneFOV()
    if MainTable.Combat.AimAssist.Main.Enabled and MainTable.Combat.AimAssist.Main.UseDeadzone then
        DeadzoneFov.Visible = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Enabled
    else
        DeadzoneFov.Visible = false
    end
    DeadzoneFov.Radius = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Radius
    DeadzoneFov.Thickness = 1
    DeadzoneFov.Color = MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.Color
    DeadzoneFov.Position = v2new(Mouse.X+MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetX, Mouse.Y+35+MainTable.Combat.AimAssist.Visuals.DeadzoneFOV.OffsetY)
end

function VisibleCheck(part, ignore) 
    if table.find(MainTable.Combat.AimAssist.Main.Checks, "Visible") then
    ...
    function WallCheck(part, ignore) 
        if table.find(MainTable.Combat.AimAssist.Main.Checks, "Wall") then
            Origin = plrs.LocalPlayer.Character:FindFirstChild("Head").Position
            CheckRay = Ray.new(Origin, part - Origin)
            Hit = workspace:FindPartOnRayWithIgnoreList(CheckRay, ignore)
            return Hit == nil 
        ...
        function ArmCheck(part, ignore) 
            if table.find(MainTable.Combat.AimAssist.Main.Checks, "Arm") then
                if GetHumanoidRNumbers(game.Players.LocalPlayer) == "R6" then
                    Origin = plrs.LocalPlayer.Character["Right Arm"].Position
                elseif GetHumanoidRNumbers(game.Players.LocalPlayer) == "R15" then
                    Origin = plrs.LocalPlayer.Character["RightHand"].Position
                end
                CheckRay = Ray.new(Origin, part - Origin)
                Hit = workspace:FindPartOnRayWithIgnoreList(CheckRay, ignore)
                return Hit == nil 
    ...
    function ForceFieldCheck(Plr)
        if table.find(MainTable.Combat.AimAssist.Main.Checks, "ForceField") then
            if not Plr.Character:FindFirstChildOfClass("ForceField") then
                return true
            else
                return false
...
function DeadzoneCheck(Distance)
    if MainTable.Combat.AimAssist.Main.UseDeadzone and MainTarget then
        if Distance < DeadzoneFov.Radius then
            return true
        else
            return false
        end
...
function GetClosestPlayerToMouse()
    ...
    for i,v in pairs(plrs:GetPlayers()) do
        pcall(function()
        if v.Character and v.Character:FindFirstChildOfClass("Humanoid") and v.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 and v.Character.HumanoidRootPart and VisibleCheck(v.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position,{plrs.LocalPlayer.Character,v.Character}) and WallCheck(v.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position,{plrs.LocalPlayer.Character,v.Character}) and ArmCheck(v.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position,{plrs.LocalPlayer.Character,v.Character}) and v ~= plrs.LocalPlayer and ForceFieldCheck(v) then
            local pos,onscreen = CC:WorldToScreenPoint(v.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position)
            if onscreen then
                local distance = (v2new(pos.X, pos.Y) - Fov.Position).magnitude
                if distance < Fov.Radius and DeadzoneCheck(distance) and distance < Closest then
                    Closest = distance
                    Target = v
                end
            end
...
function AimAtTarget(Hitbox)
    ...
    mousemoverel((WTSP.X - Mouse.X+MainTable.Combat.AimAssist.Main.OffsetX)/MainTable.Combat.AimAssist.Main.HorizontalSmoothness, (WTSP.Y - Mouse.Y+MainTable.Combat.AimAssist.Main.OffsetY)/MainTable.Combat.AimAssist.Main.VerticalSmoothness)
end
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    if MainTable.Combat.AimAssist.Main.Enabled then
        ...
        if MainTarget and getgenv().FrameVisible ~= false and plrs.LocalPlayer.Character ~= nil then
            local DistanceRange = (plrs.LocalPlayer.Character:FindFirstChild("HumanoidRootPart").Position - MainTarget.Character:FindFirstChild("HumanoidRootPart").Position).magnitude
            if DistanceRange <= MainTable.Combat.AimAssist.Main.Range then
            if table.find(MainTable.Combat.AimAssist.Main.Checks, "Team") then
                if MainTarget.TeamColor ~= plrs.LocalPlayer.TeamColor then
            if MainTable.Combat.AimAssist.Tracing.Prediction.Enabled then
                if MainTable.Combat.AimAssist.Tracing.Prediction.BackTracing then
                    AimAtTarget(MainTarget.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position-MainTarget.Character[MainTable.Combat.AimAssist.Main.Hitbox].Velocity * MainTable.Combat.AimAssist.Tracing.Prediction.Amount)
                else
            AimAtTarget(MainTarget.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position+MainTarget.Character[MainTable.Combat.AimAssist.Main.Hitbox].Velocity * MainTable.Combat.AimAssist.Tracing.Prediction.Amount)
                end
            else
            AimAtTarget(MainTarget.Character[MainTable.Combat.AimAssist.Main.Hitbox].Position)
            end
        end
        else
            if MainTable.Combat.AimAssist.Tracing.Prediction.Enabled then
    ...
    WatermarkFPS = WatermarkFPS + 1

    local pingvalue = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
    local split = string.split(pingvalue,'(')
    local ping = tonumber(split[1])
    WatermarkPing = math.round(ping)

    if MainTable.ExtraVisuals.LocalPlayer.CustomMeshes.Headless then
        game.Players.LocalPlayer.Character:FindFirstChild("Head").Transparency = 1
        for i,v in pairs(game.Players.LocalPlayer.Character:FindFirstChild("Head"):GetChildren()) do
            if (v:IsA("Decal")) then
                v.Transparency = 1
            end
        end
    ...
    if MainTable.Misc.Movement.AutoJump.Enabled then
        sethiddenproperty(plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid"), "JumpReplicate", true)
    end

    UpdateFOV()
    UpdateFOVOutline()
    UpdateDeadzoneFOV()
    UpdateDeadzoneFOVOutline()

local pass =
MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Enabled and lplr.Character and lplr.Character:FindFirstChildOfClass("Humanoid") and lplr.Character:FindFirstChild("Head") ~= nil and
(camera.CFrame.p - camera.Focus.p).magnitude > 1 and
lplr.Character:FindFirstChildOfClass("Humanoid").Health > 0
for i = 1, #drawings do
local line, triangle = drawings[i][1], drawings[i][2]
if pass then
    local color =
        MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Rainbow and Color3.fromHSV((tick() % 5 / 5 - (i / #drawings)) % 1, .5, 1) or
        MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Color
    local pos = lplr.Character:FindFirstChild("Head").Position + Vector3.new(0, 0.75, 0)
    local topWorld = pos + Vector3.new(0, MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Height, 0)

    local last, next = (i / 25) * tau, ((i + 1) / 25) * tau
    local lastWorld =
        pos + (Vector3.new(math.cos(last), 0, math.sin(last)) * 1)
    local nextWorld =
        pos + (Vector3.new(math.cos(next), 0, math.sin(next)) * 1)
    ...
    line.From = v2new(lastScreen.X, lastScreen.Y)
    line.To = v2new(nextScreen.X, nextScreen.Y)
    line.Color = color
    line.Transparency = 1
    line.Visible = true

    triangle.PointA = v2new(topScreen.X, topScreen.Y)
    triangle.PointB = line.From
    triangle.PointC = line.To
    triangle.Color = color
    triangle.Transparency = 0.50
...
local ESPToggle = PlayerESPSection:Toggle({name = "Enabled", def = MainTable.ESP.Main.Enabled, callback = function(v)  
    MainTable.ESP.Main.Enabled = v
end}):Keybind({name = "GigaBoui", def = MainTable.ESP.Main.Bind, mode = "toggle", flag = "ESPKeybind"})

local DistanceCheckAmount = PlayerESPSection:Slider({name = "Max Rendering Distance", min = 1, max = 2500, dec = 1, suf = "m", def = MainTable.ESP.Main.MaxRenderingDistance, callback = function(v)
    MainTable.ESP.Main.MaxRenderingDistance = v
end})

local ESPTeamCheck = PlayerESPSection:Toggle({name = "Team Check", def = MainTable.ESP.Checks.Team, callback = function(v)  
    MainTable.ESP.Checks.Team = v
end})

local Name = PlayerESPSection:Toggle({name = "Name", def = MainTable.ESP.Name.Enabled, callback = function(v)  
    MainTable.ESP.Name.Enabled = v
end})
local NameColor = Name:Colorpicker({name = "Bruh", def = MainTable.ESP.Name.Color, callback = function(v)
    MainTable.ESP.Name.Color = v
end})

local Box = PlayerESPSection:Toggle({name = "Bounding Box", def = MainTable.ESP.Box.Enabled, callback = function(v)  
    MainTable.ESP.Box.Enabled = v
end})
local BoxColor = Box:Colorpicker({name = "Bruh", def = MainTable.ESP.Box.Color, callback = function(v)
    MainTable.ESP.Box.Color = v
end})
BoxColor:Colorpicker({name = "Bruh", def = MainTable.ESP.Box.OutlineColor, callback = function(v)
    MainTable.ESP.Box.OutlineColor = v
end})

PlayerESPSection:Toggle({name = "Fill", def = MainTable.ESP.Box.Fill, callback = function(v)  
    MainTable.ESP.Box.Fill = v
end}):Colorpicker({name = "Bruh", def = MainTable.ESP.Box.FillColor, trans = true, defaultrans = MainTable.ESP.Box.FillTransparency/100, callback = function(v)
    for i,t in pairs(v) do
        if i == 1 then
            MainTable.ESP.Box.FillColor = t
        elseif i == 2 then
            MainTable.ESP.Box.FillTransparency = t
        end
    end
end})

local HealthBar = PlayerESPSection:Toggle({name = "Health Bar", def = MainTable.ESP.HealthBar.Enabled, callback = function(v)  
    MainTable.ESP.HealthBar.Enabled = v
end})
local HealthBarColor = HealthBar:Colorpicker({name = "Bruh", def = MainTable.ESP.HealthBar.HighColor, callback = function(v)
    MainTable.ESP.HealthBar.HighColor = v
end}):Colorpicker({name = "Bruh", def = MainTable.ESP.HealthBar.LowColor, callback = function(v)
    MainTable.ESP.HealthBar.LowColor = v
end})

local HealthText = PlayerESPSection:Toggle({name = "Health Number", def = MainTable.ESP.HealthNumber.Enabled, callback = function(v)  
    MainTable.ESP.HealthNumber.Enabled = v
end})
local HealthTextColor = HealthText:Colorpicker({name = "Bruh", def = MainTable.ESP.HealthNumber.Color, callback = function(v)
    MainTable.ESP.HealthNumber.Color = v
end})

PlayerESPSection:Toggle({name = "Distance", def = MainTable.ESP.Distance.Enabled, callback = function(v)  
    MainTable.ESP.Distance.Enabled = v
end}):Colorpicker({name = "Bruh", def = MainTable.ESP.Distance.Color, callback = function(v)
    MainTable.ESP.Distance.Color = v
end})

local WeaponText = PlayerESPSection:Toggle({name = "Weapon", def = MainTable.ESP.Weapon.Enabled, callback = function(v)  
    MainTable.ESP.Weapon.Enabled = v
end})
local WeaponColor = WeaponText:Colorpicker({name = "Bruh", def = MainTable.ESP.Weapon.Color, callback = function(v)
    MainTable.ESP.Weapon.Color = v
end})

local PlayerOverlaySection = VisualsTab:Section({name = "Player Overlay", side = "left"})

PlayerOverlaySection:Toggle({name = "Enabled", def = MainTable.ESP.PlayerOverlay.Main.Enabled, callback = function(v)  
    MainTable.ESP.PlayerOverlay.Main.Enabled = v
end})

PlayerOverlaySection:Toggle({name = "Chams", def = MainTable.ESP.PlayerOverlay.Chams.Enabled, callback = function(v)  
    MainTable.ESP.PlayerOverlay.Chams.Enabled = v
end}):Colorpicker({name = "Bruh", def = MainTable.ESP.PlayerOverlay.Chams.Color, callback = function(v)
    MainTable.ESP.PlayerOverlay.Chams.Color = v
end})

PlayerOverlaySection:Toggle({name = "Glow", def = MainTable.ESP.PlayerOverlay.Glow.Enabled, callback = function(v)  
    MainTable.ESP.PlayerOverlay.Glow.Enabled = v
end}):Colorpicker({name = "Bruh", def = MainTable.ESP.PlayerOverlay.Glow.Color, callback = function(v)
    MainTable.ESP.PlayerOverlay.Glow.Color = v
end})

PlayerOverlaySection:Dropdown({name = "Depth Mode", options = {"Always", "Occluded"}, def = MainTable.ESP.PlayerOverlay.Main.DepthMode, multi = false, scrollable = false, vis = 2, min = 0, callback = function(y)
    if y == "Always" then
        MainTable.ESP.PlayerOverlay.Main.DepthMode = "AlwaysOnTop"
    else
        MainTable.ESP.PlayerOverlay.Main.DepthMode = "Occluded"
    end
end})

PlayerOverlaySection:Toggle({name = "Health Based", def = MainTable.ESP.PlayerOverlay.Chams.HealthBasedColor, callback = function(v)  
    MainTable.ESP.PlayerOverlay.Chams.HealthBasedColor = v
end})

PlayerOverlaySection:Toggle({name = "Team Check", def = MainTable.ESP.PlayerOverlay.Checks.Team, callback = function(v)  
    MainTable.ESP.PlayerOverlay.Checks.Team = v
end})

function TeamCheck(v)
    if MainTable.ESP.Checks.Team then
        if v.TeamColor ~= plrs.LocalPlayer.TeamColor then
            return true
        else
            return false
...
function Lerp(Value, LowColor, HighColor)
    if Value <= 0 then return LowColor end
    if Value >= 100 then return HighColor end
    --
    return Color3.new(
        LowColor.R + (HighColor.R - LowColor.R) * Value,
        LowColor.G + (HighColor.G - LowColor.G) * Value,
        LowColor.B + (HighColor.B - LowColor.B) * Value
    )
end

getgenv().DrawingObjects = {}

function FindFirstPlayerToESP(Player)
    local BoxFilled = Draw("Square")
    BoxFilled.Visible = false
    BoxFilled.Filled = true
    BoxFilled.Color = Color3.new(0,0,0)
    table.insert(DrawingObjects, BoxFilled)
    ...
    local HealthBarOutline = Draw("Square")
    HealthBarOutline.Visible = false
    HealthBarOutline.Filled = true
    HealthBarOutline.Color = Color3.new(0,0,0)
    table.insert(DrawingObjects, HealthBarOutline)

    local HealthBar = Draw("Square")
    HealthBar.Visible = false
    HealthBar.Filled = true
    HealthBar.Color = Color3.new(0,1,0)
    table.insert(DrawingObjects, HealthBar)

    local HealthNumber = Draw("Text")
    HealthNumber.Visible = false
    HealthNumber.Outline = true
    HealthNumber.Font = 2
    HealthNumber.Size = 13
    HealthNumber.Text = "N/A"
    HealthNumber.Color = Color3.new(1,1,1)
    table.insert(DrawingObjects, HealthNumber)
    ...
    table.insert(DrawingObjects, Distance)
    ...
    Name.Center = true
    table.insert(DrawingObjects, Name)

    local Weapon = Draw("Text")
    ...
    Weapon.Center = true
    table.insert(DrawingObjects, Weapon)

    utility:Connect(rs.RenderStepped, function()
    if Player ~= Players.LocalPlayer and Player.Character ~= nil and Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") and TeamCheck(Player) and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 and Player.Character:FindFirstChild("HumanoidRootPart") then
        local RootPort, OnScreen = Camera:WorldToViewportPoint(Player.Character:FindFirstChild("HumanoidRootPart").Position)
        local HeadPort = Camera:WorldToViewportPoint(Player.Character:FindFirstChild("Head").Position)
        local LegPort = Camera:WorldToViewportPoint(Player.Character:FindFirstChild("HumanoidRootPart").Position - Vector3.new(0,4.5,0))
        local RoundedDistance = (Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart").Position - Player.Character:FindFirstChild("HumanoidRootPart").Position).magnitude

        if OnScreen and RoundedDistance <= MainTable.ESP.Main.MaxRenderingDistance then
        if MainTable.ESP.Main.Enabled then

            BoxOutline.Size = v2new(3200 / RootPort.Z, HeadPort.Y - LegPort.Y)
            Box.Size = BoxOutline.Size

            BoxOutline.Position = v2new(RootPort.X - BoxOutline.Size.X / 2, RootPort.Y - BoxOutline.Size.Y / 1.75)
            Box.Position = v2new(RootPort.X - Box.Size.X / 2, RootPort.Y - Box.Size.Y / 1.75)

            BoxFilled.Size = v2new(3200 / RootPort.Z, HeadPort.Y - LegPort.Y)
    
            BoxFilled.Position = v2new(RootPort.X - BoxOutline.Size.X / 2, RootPort.Y - BoxOutline.Size.Y / 1.75)

            BoxFilled.Transparency = MainTable.ESP.Box.FillTransparency


            HealthBarOutline.Size = v2new(4, HeadPort.Y-4 - LegPort.Y)
            HealthBar.Size = v2new(2, (HeadPort.Y-2 - LegPort.Y) / (Player.Character:FindFirstChildOfClass("Humanoid").MaxHealth / math.clamp(Player.Character:FindFirstChildOfClass("Humanoid").Health, 0, Player.Character:FindFirstChildOfClass("Humanoid").MaxHealth)))

            HealthBarOutline.Position = v2new(Box.Position.X - 7, Box.Position.Y+2 + (1 / HealthBarOutline.Size.Y))
            HealthBar.Position = v2new(Box.Position.X - 6, Box.Position.Y+1 + (1 / HealthBar.Size.Y))

            if Player.Character:FindFirstChildOfClass("Humanoid").Health > 9 then
                HealthNumber.Position = v2new(HealthBar.Position.X - 18.5, HealthBar.Position.Y + HealthBar.Size.Y)
            else
                HealthNumber.Position = v2new(HealthBar.Position.X - 12.5, HealthBar.Position.Y + HealthBar.Size.Y)
            end
            HealthNumber.Text = tostring(math.round(Player.Character:FindFirstChildOfClass("Humanoid").Health))
            
            Distance.Position = v2new(RootPort.X, HealthBar.Position.Y + (1 / HealthBar.Size.Y) + 4)
            Distance.Text = tostring(math.round(RoundedDistance)).."m"

            Name.Position = v2new(RootPort.X, RootPort.Y + BoxOutline.Size.Y / 2 - 17)
            Name.Text = tostring(Player.Name)

            if MainTable.ESP.Distance.Enabled then
                Weapon.Position = v2new(RootPort.X, HealthBar.Position.Y + (1 / HealthBar.Size.Y) + 4 * 4)
            else
                Weapon.Position = v2new(RootPort.X, HealthBar.Position.Y + (1 / HealthBar.Size.Y) + 4)
            end

            local HeldTool = Player.Character:FindFirstChildOfClass("Tool") and Player.Character:FindFirstChildOfClass("Tool").Name or "None"

            Weapon.Text = tostring(HeldTool)


            if MainTable.ESP.Box.Enabled then

                BoxOutline.Visible = true
                Box.Visible = true
                if MainTable.ESP.Box.Fill then
                ...
                if MainTable.ESP.HealthBar.Enabled then
                ...
                if MainTable.ESP.HealthNumber.Enabled then

                    if Player.Character:FindFirstChildOfClass("Humanoid").Health < Player.Character:FindFirstChildOfClass("Humanoid").MaxHealth then
                ...
                if MainTable.ESP.Name.Enabled then
                ...
                if MainTable.ESP.Weapon.Enabled then
                ...
                --//Colorings--\\
                    Name.Color = MainTable.ESP.Name.Color
                    Box.Color = MainTable.ESP.Box.Color
                    BoxOutline.Color = MainTable.ESP.Box.OutlineColor
                    BoxFilled.Color = MainTable.ESP.Box.FillColor
                    HealthBar.Color = Lerp(Player.Character:FindFirstChildOfClass("Humanoid").Health / Player.Character:FindFirstChildOfClass("Humanoid").MaxHealth, MainTable.ESP.HealthBar.LowColor, MainTable.ESP.HealthBar.HighColor)
                    HealthNumber.Color = MainTable.ESP.HealthNumber.Color
                    Distance.Color = MainTable.ESP.Distance.Color
                    Weapon.Color = MainTable.ESP.Weapon.Color
...
for i,v in pairs(Players:GetPlayers()) do
    FindFirstPlayerToESP(v)
end

Players.PlayerAdded:Connect(function(Player)
    FindFirstPlayerToESP(Player)
end)

function ChamsTeamCheck(Plr)
    if MainTable.ESP.PlayerOverlay.Checks.Team then
        if Plr.TeamColor ~= plrs.LocalPlayer.TeamColor then
            return true
        else
            return false
        end
...
function ESP(Plr)
    ...
    game.RunService.RenderStepped:Connect(function()
        ...
        if Plr ~= plrs.LocalPlayer and ChamsTeamCheck(Plr) and MainTable.ESP.PlayerOverlay.Main.Enabled then
            if MainTable.ESP.PlayerOverlay.Chams.Enabled then
                Highlight.FillTransparency = 0
            else
                Highlight.FillTransparency = 1
            end
            if MainTable.ESP.PlayerOverlay.Glow.Enabled then
                Highlight.OutlineTransparency = 0
            else
                Highlight.OutlineTransparency = 1
            end

            Highlight.DepthMode = MainTable.ESP.PlayerOverlay.Main.DepthMode
        
            if MainTable.ESP.PlayerOverlay.Chams.HealthBasedColor then
                Highlight.FillColor = Lerp(Plr.Character:FindFirstChildOfClass("Humanoid").Health / Plr.Character:FindFirstChildOfClass("Humanoid").MaxHealth, Color3.new(1,0,0), Color3.new(0,1,0))
            else
                Highlight.FillColor = MainTable.ESP.PlayerOverlay.Chams.Color
            end

            Highlight.OutlineColor = MainTable.ESP.PlayerOverlay.Glow.Color
        else
            Highlight.FillTransparency = 1
            Highlight.OutlineTransparency = 1
        end
...
WorldSection:Colorpicker({name = "Ambience", def = game.Lighting.Ambient, callback = function(v)
    game.Lighting.Ambient = v
end})

WorldSection:Slider({name = "Brightness", min = 0, max = 9, dec = 1, suf = "", def = game.Lighting.Brightness, callback = function(v)
    game.Lighting.Brightness = v
end})

WorldSection:Slider({name = "Time", min = 0, max = 24, dec = 1, suf = "hr", def = math.floor(game.Lighting.ClockTime), callback = function(v)
    game.Lighting.ClockTime = v
end})

WorldSection:Slider({name = "Exposure", min = 0, max = 2, dec = 1, suf = "", def = game.Lighting.ExposureCompensation, callback = function(v)
...
WorldSection:Colorpicker({name = "Fog Color", def = game.Lighting.FogColor, callback = function(v)
    game.Lighting.FogColor = v
end})

WorldSection:Toggle({name = "Shadows", def = game.Lighting.GlobalShadows, callback = function(v)  
    game.Lighting.GlobalShadows = v
end})

local ScreenSection = VisualsTab:Section({name = "Screen", side = "right"})

local AspectRatio = ScreenSection:Toggle({name = "Aspect Ratio", def = MainTable.ExtraVisuals.Screen.AspectRatio.Enabled, callback = function(v)  
    MainTable.ExtraVisuals.Screen.AspectRatio.Enabled = v
end})

local Ratio = ScreenSection:Slider({name = "Ratio", min = 1, max = 120, dec = 1, suf = "%", def = MainTable.ExtraVisuals.Screen.AspectRatio.Ratio, callback = function(v)
    MainTable.ExtraVisuals.Screen.AspectRatio.Ratio = v
end})

ScreenSection:Slider({name = "Field Of View", min = 1, max = 120, dec = 1, suf = "", def = math.round(game:GetService("Workspace"):FindFirstChildOfClass("Camera").FieldOfView), callback = function(v)
    game:GetService("Workspace"):FindFirstChildOfClass("Camera").FieldOfView = v
end})

ScreenSection:Toggle({name = "Client 3D Rendering", def = true, callback = function(v)  
    game:GetService("RunService"):Set3dRenderingEnabled(v)
end})

OldNewIndex = hookmetamethod(game, "__newindex", function(self, Key, Value)
    if (self == game:GetService("Workspace"):FindFirstChildOfClass("Camera") and Key == "CFrame") and MainTable.ExtraVisuals.Screen.AspectRatio.Enabled then 
        local X, Y, Z, R00, R01, R02, R10, R11, R12, R20, R21, R22 = Value:GetComponents()
        Value = CFrame.new(X, Y, Z, R00, R01 * MainTable.ExtraVisuals.Screen.AspectRatio.Ratio / 100, R02, R10, R11 * MainTable.ExtraVisuals.Screen.AspectRatio.Ratio / 100, R12, R20, R21 * MainTable.ExtraVisuals.Screen.AspectRatio.Ratio / 100, R22)
    end
    return OldNewIndex(self, Key, Value)
end)

local LPSection = VisualsTab:Section({name = "Local Player", side = "right"})

LPSection:Dropdown({name = "Character", options = {"Default", "Stewie Griffin"}, def = "Default", mutli = false, scrollable = false, vis = 2, min = 2, callback = function(y)
    if y == "Default" then
        if getgenv().fuc or getgenv().fuck or getgenv().W then
        fuc.Transparency = 1
        for i,v in pairs(plrs.LocalPlayer.Character:GetDescendants()) do 
    ...
    elseif y == "Stewie Griffin" then
getgenv().fuc = Instance.new("Part",workspace) 
fuc.CFrame = plrs.LocalPlayer.Character.HumanoidRootPart.CFrame
fuc.CanCollide = false
getgenv().fuck = Instance.new("SpecialMesh")
fuck.Parent = fuc
...
Weld(plrs.LocalPlayer.Character.HumanoidRootPart,fuc)
fuc.Transparency = 0
        for i,v in pairs(plrs.LocalPlayer.Character:GetDescendants()) do 
            if v:IsA("BasePart") or v:IsA("Decal") then 
                v.Transparency = 1 
...
local ThirdPerson = LPSection:Toggle({name = "Third Person", def = MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Enabled, callback = function(v)  
    MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Enabled = v
end}):Keybind({name = "Sexybot", def = MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Bind, mode = "toggle", flag = "ThirdPersonKey"})

local ThirdPersonDistance = LPSection:Slider({name = "Distance", min = 1, max = 300, dec = 1, suf = "studs", def = MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Distance, callback = function(v)
    MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Distance = v/10
end})
...
old_new_index = hookmetamethod(game, "__newindex", function(self, index, value)
    ...
    if MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Enabled == true then
        if self == game:GetService("Workspace"):FindFirstChildOfClass("Camera") and index == "CFrame" then
            value *= CFrame.new(0,0,MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Distance)
        end
    end
...
local ChinaHat = LPSection:Toggle({name = "China Hat", def = MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Enabled, callback = function(v)  
    MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Enabled = v
end})
local HatColor = ChinaHat:Colorpicker({name = "Bruh", def = MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Color, callback = function(v)
    MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Color = v
end})

local HatRainbow = LPSection:Toggle({name = "Rainbow", def = MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Rainbow, callback = function(v)  
    MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Rainbow = v
end})

local HatHeight = LPSection:Slider({name = "Height", min = 1, max = 100, dec = 1, suf = "", def = MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Height, callback = function(v)
    MainTable.ExtraVisuals.LocalPlayer.ChinaHat.Height = v/100
end})

LPSection:Toggle({name = "Headless", def = MainTable.ExtraVisuals.LocalPlayer.CustomMeshes.Headless, callback = function(v)  
    MainTable.ExtraVisuals.LocalPlayer.CustomMeshes.Headless = v
end})
...
local PlayersDropdown = PlayersSection:List({name = "Selection", options = GetCustomPlayers(), def = LocalPlayer.Name, mutli = false, scrollable = true, vis = 6, min = 6, flag = "CurrentPlayer"})

plrs.PlayerAdded:Connect(function(Player)
    PlayersDropdown:Refresh(GetCustomPlayers())
end)
...
PlayersSection:Button({name = "Teleport", callback = function()
    if library.flags["CurrentPlayer"] then
        local PlayerCFrame = plrs[library.flags["CurrentPlayer"]].Character.HumanoidRootPart.CFrame
        plrs.LocalPlayer.Character.HumanoidRootPart.CFrame = PlayerCFrame
    end
end})
...
PlayersSection:Button({name = "View", callback = function(c)
    if library.flags["CurrentPlayer"] then
        ...
        Camera.CameraSubject = plrs[library.flags["CurrentPlayer"]].Character:FindFirstChildOfClass("Humanoid")
    end
end})
...
PlayersSection:Button({name = "Copy User Id", callback = function(c)
    if library.flags["CurrentPlayer"] then
        ...
        setclipboard(tostring(plrs[library.flags["CurrentPlayer"]].UserId))
    end
end})

PlayersSection:Button({name = "Highlight", callback = function(c)
    if library.flags["CurrentPlayer"] then
        local CurrentPlayer = plrs[library.flags["CurrentPlayer"]]

        if CurrentPlayer.Character ~= nil and CurrentPlayer.Character:FindFirstChildOfClass("Humanoid") and CurrentPlayer.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
            if CurrentPlayer.Character:FindFirstChildOfClass("Highlight") then
...
local StrafeToggle = PlayersSection:Toggle({name = "Strafe", def = MainTable.Misc.Players.Strafe, callback = function(c) 
    MainTable.Misc.Players.Strafe = c
end})

PlayersSection:Slider({name = "Length", min = 0, max = 20, dec = 1, suf = "m", flag = "StrafeRadius"})

PlayersSection:Slider({name = "Height", min = 0, max = 20, dec = 1, suf = "m", flag = "sRadius"})

Loop(0, function()
    pcall(function()
    if MainTable.Misc.Players.Strafe and library.flags["CurrentPlayer"] and plrs[library.flags["CurrentPlayer"]] ~= plrs.LocalPlayer and plrs[library.flags["CurrentPlayer"]].Character ~= nil and plrs.LocalPlayer.Character ~= nil then
        for i = 1, 360 do
            local Angle = CFrame.Angles(0, math.rad(i), 0) * CFrame.new(0, library.flags["sRadius"], library.flags["StrafeRadius"])
        
            plrs.LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame = CFrame.new(plrs[library.flags["CurrentPlayer"]].Character:FindFirstChild("HumanoidRootPart").Position) * Angle
            task.wait()
            end
        else
            StrafeToggle:Set(false)
        end
    end)
end)
...
local SpeedMultiplier = MovementSection:Toggle({name = "Speed Multiplier", def = MainTable.Misc.Movement.SpeedMultiplier.Enabled, callback = function(c) 
    MainTable.Misc.Movement.SpeedMultiplier.Enabled = c
end}):Keybind({name = "Gigabot", def = MainTable.Misc.Movement.SpeedMultiplier.Bind, mode = "toggle", flag = "SpeedKey"})

local SpeedLoop = game:GetService("RunService").Heartbeat:Connect(function()
    if MainTable.Misc.Movement.SpeedMultiplier.Enabled then
    plrs.LocalPlayer.Character.HumanoidRootPart.CFrame = plrs.LocalPlayer.Character.HumanoidRootPart.CFrame + plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").MoveDirection * MainTable.Misc.Movement.SpeedMultiplier.Speed
    end
end)

local Speed = MovementSection:Slider({name = "Speed Amount", min = 0, max = 100, dec = 1, suf = "%", def = MainTable.Misc.Movement.SpeedMultiplier.Speed, callback = function(v)
    MainTable.Misc.Movement.SpeedMultiplier.Speed = v/20
end})

local InfiniteJump = MovementSection:Toggle({name = "Infinite Jump", def = MainTable.Misc.Movement.InfiniteJump.Enabled, callback = function(c) 
    MainTable.Misc.Movement.InfiniteJump.Enabled = c
end})

local JumpBug = MovementSection:Toggle({name = "Auto Jump", def = MainTable.Misc.Movement.AutoJump.Enabled, callback = function(c) 
    MainTable.Misc.Movement.AutoJump.Enabled = c
end})

if GetHumanoidRNumbers(game.Players.LocalPlayer) == "R6" then
    MovementSection:Toggle({name = "Swim Fly (Space)", def = false, callback = function(c) 
        if c == true then
            getgenv().SwimConnection = game:GetService("RunService").Heartbeat:Connect(function()
                if game.Players.LocalPlayer.Character ~= nil and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
                    ChangeHumanoidStateType(Enum.HumanoidStateType.Swimming)
                end
            end)
        else
...
local Strafe = MovementSection:Toggle({name = "Strafe", def = MainTable.Misc.Movement.Strafe.Enabled, callback = function(c) 
    MainTable.Misc.Movement.Strafe.Enabled = c
end})

local StrafeSpeed = MovementSection:Slider({name = "Strafe Speed", min = 1, max = 200, dec = 1, suf = "", def = MainTable.Misc.Movement.Strafe.Speed, callback = function(v)
    MainTable.Misc.Movement.Strafe.Speed = v
end})

uis.InputBegan:Connect(function(Key, Event)
    if MainTable.Misc.Movement.InfiniteJump.Enabled then
    if (Key.KeyCode == Enum.KeyCode.Space) then
        if plrs.LocalPlayer.Character ~= nil and plrs.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
            plrs.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").JumpPower, 0)
            end
        end
    end
end)

local function AIsKeyDown()
	return uis:IsKeyDown(Enum.KeyCode.W) or uis:IsKeyDown(Enum.KeyCode.Space)
end

local function input(Key, Event)
    if AIsKeyDown() then
        if MainTable.Misc.Movement.Strafe.Enabled then
        if plrs.LocalPlayer.Character ~= nil and plrs.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
            if plrs.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Jump then
                plrs.LocalPlayer.Character.HumanoidRootPart.Velocity = plrs.LocalPlayer.Character.HumanoidRootPart.CFrame.lookVector * MainTable.Misc.Movement.Strafe.Speed
                end
            end
        end
    end
...
local NetworkSection = MiscTab:Section({name = "Network", side = "right"})

NetworkSection:Toggle({name = "Anti AFK", def = MainTable.Misc.Network.AntiAFK.Enabled, callback = function(v)  
    MainTable.Misc.Network.AntiAFK.Enabled = v
end})

Players.LocalPlayer.Idled:Connect(function()
    if MainTable.Misc.Network.AntiAFK.Enabled then
        mouse1click()
    end
end)

local LagSwitch = NetworkSection:Toggle({name = "Lag Switch", def = MainTable.Misc.Network.LagSwitch.Enabled, callback = function(c) 
    MainTable.Misc.Network.LagSwitch.Enabled = c
end})

game:GetService("RunService").Heartbeat:Connect(function()
    if plrs.LocalPlayer.Character ~= nil and plrs.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
    if MainTable.Misc.Network.LagSwitch.Enabled then
...
NetworkSection:Toggle({name = "Fake Lag", def = MainTable.Misc.Network.FakeLag.Enabled, callback = function(c) 
    ...
    MainTable.Misc.Network.FakeLag.Enabled = c
    ...
    while MainTable.Misc.Network.FakeLag.Enabled do
        wait()
        local loop = RunService.Heartbeat:Connect(function()
            local SelfCharacter = LocalPlayer.Character
            sethiddenproperty(SelfCharacter:FindFirstChild("HumanoidRootPart"), "NetworkIsSleeping", true)
    wait(0.01)
        sethiddenproperty(SelfCharacter:FindFirstChild("HumanoidRootPart"), "NetworkIsSleeping", false)
        wait(0.01)
        end)
        wait(MainTable.Misc.Network.FakeLag.Ticks)
        if loop then
            loop:Disconnect()
        end
    
        if MainTable.Misc.Network.FakeLag.Enabled == false then
            if loop then
                loop:Disconnect()
            end
        end
...
NetworkSection:Slider({name = "Ticks", min = 1, max = 16, dec = 1, suf = "", def = MainTable.Misc.Network.FakeLag.Ticks, callback = function(v)
    MainTable.Misc.Network.FakeLag.Ticks = v/100
end})

local AutismSection = MiscTab:Section({name = "Autism", side = "left"})

local AutoSoundPlayer = AutismSection:Toggle({name = "Auto Sound Player (Autism)", def = MainTable.Misc.Sound.ASP.Enabled, callback = function(c) 
    MainTable.Misc.Sound.ASP.Enabled = c
end})

local ASPVolume = AutismSection:Slider({name = "Volume", min = 1, max = 100, dec = 1, suf = "mhz", def = MainTable.Misc.Sound.ASP.Volume, callback = function(v)
    MainTable.Misc.Sound.ASP.Volume = v/10
end})

AutismSection:Button({name = "Count to 10", callback = function()
    
    Window.Notifications.notiflist:new({text = "Counting to 10.", dur = 3, sdb = true, pulse = false})
...
Loop(1.5, function()
    if MainTable.Misc.Sound.ASP.Enabled == true then
        local RandomSound = math.random(1,7)
        local a = Instance.new("Sound")
        a.Parent = game.CoreGui
        a.Volume = MainTable.Misc.Sound.ASP.Volume
        if RandomSound == 1 then
            a.SoundId = getsynasset("lobby.mp3")
        elseif RandomSound == 2 then
            a.SoundId = getsynasset("beanbot.mp3")
        elseif RandomSound == 3 then
            a.SoundId = getsynasset("autism.mp3")
        elseif RandomSound == 4 then
            a.SoundId = getsynasset("yelling.mp3")
        elseif RandomSound == 5 then
            a.SoundId = getsynasset("meme-pssy.mp3")
        elseif RandomSound == 6 then
            a.SoundId = getsynasset("meme.mp3")
        elseif RandomSound == 7 then
            a.SoundId = getsynasset("hog-rider.mp3")
        end
        a:Play()
    end
end)
...
Window.theme = MainTable.UI.Theme
Window:Update()

MenuSection:Colorpicker({name = "Accent", def = Window.theme.accent, callback = function(v)
    Window.theme.accent = v
    Window:Update()
end})

MenuSection:Colorpicker({name = "Dark Contrast", def = Window.theme.dcont, callback = function(v)
    Window.theme.dcont = v
    Window:Update()
end})

MenuSection:Colorpicker({name = "Light Contrast", def = Window.theme.lcont, callback = function(v)
    Window.theme.lcont = v
    Window:Update()
end})

MenuSection:Colorpicker({name = "Contrast", def = Window.theme.cont, callback = function(v)
    Window.theme.cont = v
    Window:Update()
end})

MenuSection:Colorpicker({name = "Outline", def = Window.theme.outline, callback = function(v)
    Window.theme.outline = v
    Window:Update()
end})

MenuSection:Colorpicker({name = "Second Outline", def = Window.theme.outline2, callback = function(v)
    Window.theme.outline2 = v
end})

Window:Watermark()
Window:Keybinds()

MenuSection:Toggle({name = "Watermark", def = MainTable.UI.Frames.Watermark, callback = function(v)  
    Window.watermark:SetVisible(v)
end})

MenuSection:Toggle({name = "Keybinds", def = MainTable.UI.Frames.KeybindList, callback = function(v)  
        Window.kblist:ShowHideFromMyLifePleaseSomebodyKillMeIDontWantToBeAliveRightNowImUselessInMyLife(v)
end})

MenuSection:Button({name = "Close Client UI", callback = function() 
    Window:Unload()

    uis.MouseIconEnabled = true

    loadfile("pintohaxx/Configs/Default")()
    ...
    game.CoreGui.Keystroke:Destroy()
...
local GameSection = SettingsTab:Section({name = "Game", side = "right"})

GameSection:Button({name = "Rejoin Server", callback = function()
    TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId)
end})

getgenv().MaxServerPing = 0

GameSection:Slider({name = "Max Client Ping", min = 0, max = 300, dec = 1, suf = "ms", def = 0, callback = function(v)
    getgenv().MaxClientPing = v/2
end})

GameSection:Slider({name = "Min Physics Frames", min = 0, max = 59, dec = 1, suf = "/s", def = 0, callback = function(v)
    getgenv().MinPhysicsFPS = v
end})

GameSection:Button({name = "Change Server", callback = function()
    local x = {}
    for _, v in ipairs(game:GetService("HttpService"):JSONDecode(game:HttpGetAsync("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")).data) do
        if type(v) == "table" and v.maxPlayers > v.playing and v.id ~= game.JobId then
            local ping = v.ping
            local fps = v.fps
            if ping < MaxClientPing and fps > MinPhysicsFPS then
                x[#x + 1] = v.id
            end
        end
    end
...
GameSection:Button({name = "Exit Client", callback = function()
    game:Shutdown()
end})

Loop(0, function()
    local OldFlag = library.flags["AimbotKeybind"]
    local OldFlag2 = library.flags["ESPKeybind"]
    local OldFlag3 = library.flags["SpeedKey"]
    local OldFlag4 = library.flags["ThirdPersonKey"]
    local OldFlag5 = library.flags["TriggerBotBind"]

    game:GetService("RunService").RenderStepped:Wait()

    if library.flags["AimbotKeybind"] ~= OldFlag then
        MainTable.Combat.AimAssist.Main.Enabled = library.flags["AimbotKeybind"]
    end
    if library.flags["ESPKeybind"] ~= OldFlag2 then
        MainTable.ESP.Main.Enabled = library.flags["ESPKeybind"]
    end
    if library.flags["SpeedKey"] ~= OldFlag3 then
        MainTable.Misc.Movement.SpeedMultiplier.Enabled = library.flags["SpeedKey"]
    end
    if library.flags["ThirdPersonKey"] ~= OldFlag4 then
        MainTable.ExtraVisuals.LocalPlayer.ThirdPerson.Enabled = library.flags["ThirdPersonKey"]
    end
    if library.flags["TriggerBotBind"] ~= OldFlag5 then
        MainTable.Combat.TriggerBot.Main.Enabled = library.flags["TriggerBotBind"]
    end
end)
